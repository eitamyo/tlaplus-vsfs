...
\*            if (op = "CreateFile") {
        if(current_op = "none"){
            ...
            if (create_stage = "idle" /\ ((\A i \in InodeIds : inodes[i].valid) \/ (DOMAIN dir = FileNames))){
                current_op := "none";
            }
            else if (create_stage = "idle"){
            ...
            if (delete_stage = "idle" /\ DOMAIN dir = {}){
                current_op := "none";
            }
            else if (delete_stage = "idle") {
...
Next == IF current_op = "none"
           THEN /\ \/ /\ current_op' = "create"
                ...
                /\ UNCHANGED << freeBlocks, inodes, dir, curFileName, 
                                ...
                                write_stage, delete_stage, curDeletedBlocks >>
           ELSE /\ IF current_op = "create"
                      THEN /\ IF create_stage = "idle" /\ ((\A i \in InodeIds : inodes[i].valid) \/ (DOMAIN dir = FileNames))
                                 THEN /\ current_op' = "none"
                                      /\ UNCHANGED << inodes, dir, 
                                                      curFileName, curINode, 
                                                      create_stage >>
                                 ELSE /\ IF create_stage = "idle"
                      ...
                      ELSE /\ IF current_op = "write"
                                 THEN /\ IF write_stage = "idle" /\ (freeBlocks = {} \/ DOMAIN dir = {})
                                      ...
                                      /\ UNCHANGED << dir, delete_stage, 
                                                      curDeletedBlocks >>
                                 ELSE /\ IF current_op = "delete"
                                            THEN /\ IF delete_stage = "idle" /\ DOMAIN dir = {}
                                                       THEN /\ current_op' = "none"
                                                            /\ UNCHANGED << freeBlocks, 
                                                                            inodes, 
                                                                            dir, 
                                                                  ...
                                                                  ELSE /\ IF delete_stage = "remove_dir_entry"
                                                                             THEN /\ LET i == dir[curFileName] IN
                                                                                  ...
                                                                                  /\ UNCHANGED << freeBlocks, 
                                                                                                  ...
                                                                                                  curDeletedBlocks >>
                                                                             ELSE /\ IF delete_stage = "invalidate_inode"
                                                                                        THEN /\ curDeletedBlocks' = inodes[curINode].blocks
                                                                                             /\ inodes' = [inodes EXCEPT ![curINode] = [valid |-> FALSE, isDir |-> FALSE, size |-> 0, blocks |-> {}]]
                                                                                             /\ delete_stage' = "free_blocks"
                                                                                             /\ UNCHANGED << freeBlocks, 
                                                                                                             curFileName, 
                                                                                                             curINode, 
                                                                                                             current_op >>
                                                                                        ELSE /\ IF delete_stage = "free_blocks"
                                                                                                   THEN /\ freeBlocks' = (freeBlocks \cup curDeletedBlocks)
                                                                                                        /\ curDeletedBlocks' = {}
                                                                                                        /\ delete_stage' = "idle"
                                                                                                        /\ curFileName' = "None"
                                                                                                        /\ curINode' = 0
...
FileCreationWorks ==
  \A name \in FileNames : 
    [] ((current_op = "create" /\ curFileName = name) => <> (name \in DOMAIN dir))
