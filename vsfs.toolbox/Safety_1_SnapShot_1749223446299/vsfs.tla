...
\*                        }
                ...
                delete_stage := "idle"; \* Reset for the next deletion
                curFileName := "None";
             }
\*            };
        }
...
\* BEGIN TRANSLATION (chksum(pcal) = "46858b62" /\ chksum(tla) \in STRING)
VARIABLES freeBlocks, inodes, dir, curFileName, delete_stage, pc

(* define statement *)
MaxBlocksPerFile == 2


vars == << freeBlocks, inodes, dir, curFileName, delete_stage, pc >>
...
Main == /\ pc = "Main"
        /\ \/ /\ pc' = "CreateFileOp"
           ...
           \/ /\ pc' = "ReadFileOp"
           \/ /\ pc' = "DeleteFileOp"
        /\ UNCHANGED << freeBlocks, inodes, dir, curFileName, delete_stage >>

CreateFileOp == /\ pc = "CreateFileOp"
                /\ \E name \in FileNames \ {curFileName}:
                ...
                /\ pc' = "Main"
                /\ UNCHANGED << freeBlocks, curFileName, delete_stage >>
...
WriteFileOp == /\ pc = "WriteFileOp"
               ...
               /\ pc' = "Main"
               /\ UNCHANGED << dir, curFileName, delete_stage >>
                ...
                /\ IF delete_stage = "idle"
                      ...
                      ELSE /\ IF delete_stage = "free_blocks"
                                 THEN /\ LET i == dir[curFileName] IN
                                           /\ freeBlocks' = (freeBlocks \cup inodes[i].blocks)
                                           /\ delete_stage' = "invalidate_inode"
                                      /\ UNCHANGED << inodes, dir, curFileName >>
                                 ELSE /\ IF delete_stage = "invalidate_inode"
                                            THEN /\ LET i == dir[curFileName] IN
                                                 ...
                                                 /\ UNCHANGED << dir, 
                                                                 curFileName >>
                                            ELSE /\ IF delete_stage = "remove_dir_entry"
                                                       THEN /\ dir' = [d \in DOMAIN dir \ {curFileName} |-> dir[d]]
                                                            /\ delete_stage' = "idle"
                                                            /\ curFileName' = "None"
                                                       ELSE /\ TRUE
                                                            /\ UNCHANGED << dir, 
...
\* Last modified Fri Jun 06 18:23:37 IDT 2025 by eitam
