...
\* BEGIN TRANSLATION (chksum(pcal) = "cad7dca1" /\ chksum(tla) \in STRING)
...
FileCreated(name) == 
    name \in DOMAIN dir 
\*    /\ dir[name] \in DOMAIN inodes
    /\ inodes[dir[name]].valid = TRUE
AllFilesCreateCalled == 
    \A name \in FileNames: FileCreated(name)
        
Spec == /\ Init /\ [][Next]_vars
        /\ WF_vars(Next)
        /\ SF_vars(AllFilesCreateCalled)
...
\* Last modified Fri Jun 06 12:46:42 IDT 2025 by eitam
